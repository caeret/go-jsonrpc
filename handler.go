...
func doCall(methodName string, f reflect.Value, params []reflect.Value) (out []reflect.Value, err error) {
	defer func() {
		if i := recover(); i != nil {
			...
			logging.Error("do call", "error", err)
		}
	}()
...
func (s *handler) getSpan(ctx context.Context, req request) (context.Context, *trace.Span) {
	...
	if eSC, ok := req.Meta["SpanContext"]; ok {
		...
		if err != nil {
			logging.Error("SpanContext: decode", "error", err)
			return ctx, nil
		}
		sc, ok := propagation.FromBinary(bSC)
		if !ok {
			logging.Error("SpanContext: could not create span", "data", bSC)
			return ctx, nil
		}
...
func (s *handler) handle(ctx context.Context, req request, w func(func(io.Writer)), rpcError rpcErrFunc, done func(keepCtx bool), chOut chanOut) {
	...
	if handler.errOut != -1 {
		...
		if err != nil {
			logging.Warn("error in RPC call", "method", req.Method, "error", err)
	...
	if resp.Error == nil {
		if res != nil && kind == reflect.Chan {
			...
			logging.Warn("failed to setup channel in RPC call", "method", req.Method, "error", err)
			stats.Record(ctx, metrics.RPCResponseError.M(1))
	...
	if resp.Error != nil && nonZero {
		logging.Error("error and res returned", "request", req, "r.err", resp.Error, "res", res)
	}

	withLazyWriter(w, func(w io.Writer) {
		if err := json.NewEncoder(w).Encode(resp); err != nil {
			logging.Error("withLazyWriter", "error", err)
			stats.Record(ctx, metrics.RPCResponseError.M(1))
			return
		}
	})
